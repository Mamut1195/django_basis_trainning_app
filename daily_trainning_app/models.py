...
class Exercise(models.Model):
    ...
    classification = models.ForeignKey(
        Classification, on_delete=models.CASCADE, verbose_name=_("Classification")
    )
...
class UserExerciseRM(models.Model):
    ...
    fecha_registro = models.DateField(default= now, verbose_name=_("Date Recorded"))

    class Meta:
        verbose_name = _("User Exercise 1RM")
        verbose_name_plural = _("User Exercise 1RMs")
        ordering = ['-fecha_registro']  # Ordenamos del más reciente al más antiguo
    ...
    def get_latest_rm(user, exercise):
        ...
        latest_rm = UserExerciseRM.objects.filter(user=user, exercise=exercise).order_by('-fecha_registro').first()
    ...
    def get_latest_rm_from_workouts(user, exercise):
        ...
        latest_workout = WorkoutData.objects.filter(user=user, exercise=exercise).order_by('-fecha').first()
        
        if not latest_workout:
            return 0  # Si no hay entrenamientos, devolvemos 0

        # Usamos la mejor fórmula para calcular el 1RM desde el último entrenamiento
...
class WorkoutData(models.Model):
    ...
    total_reps = models.PositiveIntegerField(default=0, verbose_name=_("Total Reps"))
    peso = models.PositiveIntegerField(default=0, verbose_name=_("Weight (kg)"))
    intensidad_relativa = models.FloatField(default=0.0, verbose_name=_("Relative Intensity (%)"))
    carga = models.PositiveIntegerField(default=0, verbose_name=_("Load (kg)"))
    volumen_relativo = models.FloatField(default=0.0, verbose_name=_("Relative Volume"))
    rpe_objetivo = models.FloatField(default=0.0, verbose_name=_("Target RPE"))
    rm_sesion = models.FloatField(default=0.0, verbose_name=_("Estimated 1RM (Session)"))
    ...
    def clean(self):
        ...
        user_rm = UserExerciseRM.objects.filter(user=self.user, exercise=self.exercise).first()
        if user_rm and user_rm.peso_maximo_rm:
            self.intensidad_relativa = round((self.peso / user_rm.peso_maximo_rm) * 100, 2)
        else:
        ...
        if self.total_reps and self.intensidad_relativa:
            self.volumen_relativo = round(self.total_reps * (self.intensidad_relativa ), 2)
    ...
    def calcular_rm_sesion(self):
        ...
        factor_ajuste = factores_ajuste.get(self.exercise.nivel_fatiga, 0.028)  # Default: Medio

        # Aplicar la fórmula
        rm_estimado = ((self.reps + 10 - self.rpe_objetivo) * self.peso * factor_ajuste) + self.peso
    ...
    def calcular_rpe(self):
        ...
        rir_porcentaje_rm = {100: 0, 95: 1, 90: 2, 85: 3, 80: 4, 75: 5, 70: 6, 65: 7, 60: 8}
        intensidad = min(rir_porcentaje_rm.keys(), key=lambda x: abs(x - porcentaje_rm))
