...
class WorkoutDataInline(admin.TabularInline):  
    ...
    fields = (
        'fecha', 'exercise', 'sets', 'reps', 'total_reps', 'peso', 'carga',
        'intensidad_relativa', 'volumen_relativo', 'rpe_objetivo', 'rm_sesion', 'get_nivel_fatiga'
    )
    readonly_fields = (
        'total_reps', 'carga', 'intensidad_relativa', 'volumen_relativo', 'rpe_objetivo', 'rm_sesion', 'get_nivel_fatiga'
    ...
    def get_queryset(self, request):
        ...
        return qs.filter(fecha__gte=now() - timedelta(days=10)).order_by('-fecha')
...
class UserAdmin(admin.ModelAdmin):
    ...
    inlines = [WorkoutDataInline]  # Permite ver los entrenamientos dentro de cada usuario
...
class WorkoutDataAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'user', 'exercise', 'fecha', 'sets', 'reps', 'total_reps', 'peso',
        'intensidad_relativa', 'carga', 'volumen_relativo', 'rpe_objetivo', 'rm_sesion', 'get_nivel_fatiga'
    )
    list_filter = ('fecha', 'user', 'exercise', 'exercise__nivel_fatiga')  
    search_fields = ('user__nombre', 'exercise__nombre')  
    ordering = ('-fecha',)
    readonly_fields = ('total_reps', 'carga', 'intensidad_relativa', 'volumen_relativo', 'rpe_objetivo', 'rm_sesion')
