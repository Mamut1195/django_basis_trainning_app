...
class ExerciseSerializer(serializers.ModelSerializer):
    classification = ClassificationSerializer(read_only=True)  # Muestra detalles completos de clasificación
    classification_id = serializers.PrimaryKeyRelatedField(
        queryset=Classification.objects.all(), source='classification', write_only=True
    )  # Permite enviar solo el ID en POST

    class Meta:
        model = Exercise
        fields = ['id', 'nombre', 'video', 'descripcion', 'classification', 'classification_id']
...
class WorkoutDataSerializer(serializers.ModelSerializer):
    ...
    def validate_sets(self, value):
        ...
        if value < 0:
            raise serializers.ValidationError("Los sets deben ser un número positivo.")
        return value

    def validate_reps(self, value):
        """ Validar que las reps sean un número positivo """
        if value < 0:
            raise serializers.ValidationError("Las repeticiones deben ser un número positivo.")
        return value
